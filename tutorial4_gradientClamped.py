...
def main():
    ...
    sx = Func('sx')
    sy = Func('sy')
    sobelMagnitude=Func('sobelMagnitude')
    ...
    # In terms of images, it returns the pixel on the edge nearest to x, y
    ...
    # - pix(y-1, x-1, lumi) + pix(y-1, x+1, lumi)
    ...
    sobelMagnitude[x,y,c]= sqrt(sx[x,y,0]**2 + sy[x,y,0]**2)
    ...
    output = sobelMagnitude.realize(input.width(), input.height(), input.channels());

    outputNP=numpy.array(Image(output))
    imageIO.imwrite(outputNP, 'tutorial4.png', gamma=1.0)
